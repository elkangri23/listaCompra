...
export interface GetAuditHistoryForListDto {
  listId: string;
  page?: number;
  pageSize?: number;
...
export class GetAuditHistoryForList {
  constructor(private readonly auditRepository: IAuditRepository) {}

  async execute(dto: GetAuditHistoryForListDto): Promise<Result<AuditHistoryResponse, ValidationError | Error>> {
    if (!dto.listId) {
      return failure(ValidationError.create('listId is required', 'listId', dto.listId));
    }
